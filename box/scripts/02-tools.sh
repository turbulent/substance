...
echo 'DOCKER_OPTS="-H 0.0.0.0:2375"' | sudo tee --apend /etc/default/docker > /dev/null
echo 'export DOCKER_HOST="tcp://127.0.0.1:2375"' | sudo tee /etc/profile.d/docker.sh > /dev/null
sudo service docker restart

#Install needed packages and development tools
echo "Installing PIP and dockwrkr"
