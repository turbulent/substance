...
class Link(object):
    ...
    def connectEngine(self, engine):
        ...
        logger.debug("Connecting to engine %s via %s:%s" %
                     (engine.name, self.hostname, self.port))
        ...
        try:
            ...
            if not self.key:
                ...
                if self.keyFormat == 'DSA':
                    self.key = paramiko.dsskey.DSSKey.from_private_key_file(
                        self.keyFile, password=None)
                elif self.keyFormat == 'ECSDA':
                    self.key = paramiko.ecdsakey.ECDSAKey.from_private_key_file(
                        self.keyFile, password=None)
                else:
                    self.key = paramiko.rsakey.RSAKey.from_private_key_file(
                        self.keyFile, password=None)
