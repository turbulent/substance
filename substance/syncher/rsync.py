...
class SubwatchSyncher(BaseSyncher):
    ...
    def start(self, direction=Syncher.BOTH, path=''):
        ...
        self.remoteAgent.connect("ws://%s:%s" %
                                 (self.engine.getPublicIP(), 1500))
    ...
    def scheduleSync(self, direction):
        if self.schedule[direction] is False:
            ...
            ioloop.IOLoop.current().call_later(self.syncPeriod, defer(
                self.incrementalSync, direction=direction))
    ...
    def ignoreSync(self, direction, folder, paths=[], timeout=1):
        for path in paths:
            logger.debug("Ignoring (%s)%s for %ss" %
                         (direction, path, timeout))
    ...
    def processEvent(self, event, direction=None):
        ...
        if os.path.dirname(path) in self.synching[direction]:
            logger.debug("Ignored (synching) event of (%s)%s" %
                         (direction, path))
            return
    ...
    def initialSync(self, direction):
        ...
        for folder in folders:
            if direction in [Syncher.UP, Syncher.BOTH]:
                chain = chain.then(
                    defer(self.syncFolder, folder=folder, direction=Syncher.UP, paths=['/']))
            if direction in [Syncher.DOWN, Syncher.BOTH]:
                chain = chain.then(
                    defer(self.syncFolder, folder=folder, direction=Syncher.DOWN, paths=['/']))
        return chain

    def incrementalSync(self, direction):
        folders = self.toSync[direction].copy()
        seq = [self.syncFolder(
            folder, direction, folders[folder], incremental=True) for folder in folders]
    ...
    def syncFolder(self, folder, direction, paths, incremental=False):
        ...
        if paths:
            logger.info("Synchronizing %s %s %s:%s" % (
                folder.hostPath, direction, self.engine.name, folder.guestPath))
        ...
        if direction == Syncher.UP:
            ...
            syncher.setTo(folder.guestPath,
                          self.engine.getSSHIP(), "substance")
        elif direction == Syncher.DOWN:
            syncher.setFrom(folder.guestPath,
                            self.engine.getSSHIP(), "substance")
