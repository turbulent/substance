...
class Engine(object):
    ...
    def getDefaults(self):
        ...
        defaults['aliases'] = {alias: {'container': 'web', 'user': 'heap',
                                       'cwd': '/vol/website', 'args': [alias]} for alias in aliases}
    ...
    def validateConfig(self, config):
        fields = ['name', 'driver', 'profile',
                  'docker', 'network', 'devroot', 'box']

        ops = []
        ops.append(self.config.validateFieldsPresent(
            config.getConfig(), fields))
    ...
    def addToHostsFile(self):
        self.logAdapter.info(
            "Registering engine as '%s' in local hosts file" % self.getDNSName())
    ...
    def provision(self):
        ...
        logger.info("Provisioning engine \"%s\" with driver \"%s\"",
                    self.name, self.config.get('driver'))
    ...
    def stop(self, force=False):
        ...
        if(force):
            ...
            operation >> self.__halt >> dinfo(
                "Engine \"%s\" has been halted.", self.name)
    ...
    def exposePort(self, local_port, public_port, scheme):
        ...
        with closing(socket.socket(socket.AF_INET, socket.SOCK_DGRAM)) as s:
            ...
            if scheme:
                self.logAdapter.info(
                    "%s://%s:%s is now available.", scheme, ip, public_port)
            else:
                self.logAdapter.info(
                    "Others can now connect to port %s via IP %s.", public_port, ip)
    ...
    def envRestart(self, time=10, containers=[]):
        ...
        if len(containers) > 0:
            self.logAdapter.info("Restarting %s containers" %
                                 (' '.join(containers)))
            cmds.append('subenv run dockwrkr restart -t %s %s' %
                        (time, ' '.join(containers)))
        else:
    ...
    def envRecreate(self, time=10, containers=[]):
        ...
        if len(containers) > 0:
            self.logAdapter.info("Recreating %s containers" %
                                 (' '.join(containers)))
            cmds.append('subenv run dockwrkr recreate -t %s %s' %
                        (time, ' '.join(containers)))
        else:
    ...
    def envStop(self, time=10, containers=[]):
        ...
        if len(containers) > 0:
            self.logAdapter.info("Stopping %s container(s)" %
                                 (' '.join(containers)))
            cmds.append('subenv run dockwrkr stop -t %s %s' %
                        (time, ' '.join(containers)))
        else:
